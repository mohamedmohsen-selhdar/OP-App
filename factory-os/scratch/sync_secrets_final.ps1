...
$secrets = @(
    ...
    @{ name = "SUPABASE_URL"; value = "https://qmhgckkqksnxwmxqllkp.supabase.co" }
)

Write-Host "Syncing Secrets..." -ForegroundColor Cyan

# The API expects an ARRAY of secret objects
$body = $secrets | ConvertTo-Json
$url = "https://api.supabase.com/v1/projects/$projectId/secrets"

try {
    $result = Invoke-RestMethod -Method Post -Uri $url -Headers $headers -Body $body
    Write-Host "Success! All secrets synced." -ForegroundColor Green
} catch {
    Write-Host "Bulk failed. Trying individual POSTs..." -ForegroundColor Yellow
    foreach ($s in $secrets) {
        try {
            $singleBody = @($s) | ConvertTo-Json
            Invoke-RestMethod -Method Post -Uri $url -Headers $headers -Body $singleBody
            Write-Host "Set $($s.name)" -ForegroundColor Green
        } catch {
            Write-Host "Failed to set $($s.name): $($_.Exception.Message)" -ForegroundColor Red
            if ($_.Exception.Response) {
                $stream = $_.Exception.Response.GetResponseStream()
                $reader = New-Object System.IO.StreamReader($stream)
                Write-Host "Reason: $($reader.ReadToEnd())" -ForegroundColor Gray
            }
        }
    }
}
